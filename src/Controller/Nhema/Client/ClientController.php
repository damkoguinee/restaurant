...
namespace App\Controller\Nhema\Client;
...
#[Route('/nhema/client/client')]
class ClientController extends AbstractController
{
    #[Route('/accueil/{lieu_vente}', name: 'app_nhema_client_client_index', methods: ['GET'])]
    public function index(ClientRepository $clientRepository, LieuxVentes $lieu_vente, Request $request, UserRepository $userRep, EntrepriseRepository $entrepriseRep): Response
    {
        ...
        return $this->render('nhema/client/client/index.html.twig', [
            'clients' => $clients,
    ...
    #[Route('/new/{lieu_vente}', name: 'app_nhema_client_client_new', methods: ['GET', 'POST'])]
    public function new(Request $request, EntityManagerInterface $entityManager, LieuxVentes $lieu_vente, DeviseRepository $deviseRep, UserRepository $userRep, EntrepriseRepository $entrepriseRep, UserPasswordHasherInterface $userPasswordHasher): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($request->get('retour')) {
                ...
                return $this->redirectToRoute('app_nhema_client_client_index', ['lieu_vente' => $lieu_vente->getId()], Response::HTTP_SEE_OTHER);
            }
            
        }
        $referer = $request->headers->get('referer');
        return $this->render('nhema/client/client/new.html.twig', [
            'client' => $client,
            'form' => $form,
            'user'  => $user,
            'form_user' => $form_user,
    ...
    #[Route('/show/{id}/{lieu_vente}', name: 'app_nhema_client_client_show', methods: ['GET'])]
    public function show(Client $client, LieuxVentes $lieu_vente, EntrepriseRepository $entrepriseRep): Response
    {
        return $this->render('nhema/client/client/show.html.twig', [
            'client' => $client,
            'entreprise' => $entrepriseRep->find(1),
            'lieu_vente' => $lieu_vente,
        ]);
    ...
    #[Route('/edit/{id}/{lieu_vente}', name: 'app_nhema_client_client_edit', methods: ['GET', 'POST'])]
    public function edit(Request $request, Client $client, EntityManagerInterface $entityManager, DeviseRepository $deviseRep, Hasher $hasher, LieuxVentes $lieu_vente, EntrepriseRepository $entrepriseRep): Response
    {
        $form = $this->createForm(ClientType::class, $client);
        $form->handleRequest($request);
        $form_user = $this->createForm(UserType::class, $client->getUser());
        $form_user->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($mdp) {
                $mdpHashe=$hasher->hashPassword($client->getUser(), $mdp);
                $client->getUser()->setPassword($mdpHashe);
            }
        ...
        $referer = $request->headers->get('referer');
        return $this->render('nhema/client/client/edit.html.twig', [
            'client' => $client,
            'form' => $form,
            'entreprise' => $entrepriseRep->find(1),
            'lieu_vente' => $lieu_vente,
    ...
    #[Route('/delete/{id}/{lieu_vente}', name: 'app_nhema_client_client_delete', methods: ['POST'])]
    public function delete(Request $request, Client $client, EntityManagerInterface $entityManager, LieuxVentes $lieu_vente, EntrepriseRepository $entrepriseRep): Response
    {
