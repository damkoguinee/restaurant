...
#[Route('/nhema/caisse/transfert/fond')]
class TransfertFondController extends AbstractController
{
    #[Route('/accueil/{lieu_vente}', name: 'app_nhema_caisse_transfert_fond_index', methods: ['GET'])]
    public function index(TransfertFondRepository $transfertFondRepository, Request $request, CaisseRepository $caisseRep, LieuxVentes $lieu_vente, EntrepriseRepository $entrepriseRep): Response
    {
        ...
        return $this->render('nhema/caisse/transfert_fond/index.html.twig', [
    ...
    #[Route('/new/{lieu_vente}', name: 'app_nhema_caisse_transfert_fond_new', methods: ['GET', 'POST'])]
    public function new(Request $request, EntityManagerInterface $entityManager, LieuxVentes $lieu_vente, TransfertFondRepository $transfertRep, MouvementCaisseRepository $mouvementRep, CompteOperationRepository $compteOpRep, CategorieOperationRepository $catetgorieOpRep, EntrepriseRepository $entrepriseRep): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (empty($caisse_depart) and empty($caisse_recep)) {
                ...
                if ($referer) {
                    ...
                    return $this->render('nhema/caisse/transfert_fond/new.html.twig', [
                        'entreprise' => $entrepriseRep->find(1),
                        'lieu_vente' => $lieu_vente,
                        'form' => $formView,
                ...
                if ($solde_caisse >= $montant) {                    
                    ...
                    return $this->redirectToRoute('app_nhema_caisse_transfert_fond_index', ['lieu_vente' => $lieu_vente->getId()], Response::HTTP_SEE_OTHER);
                }else{
    ...
    #[Route('/show/{id}/{lieu_vente}', name: 'app_nhema_caisse_transfert_fond_show', methods: ['GET'])]
    public function show(TransfertFond $transfertFond, LieuxVentes $lieu_vente, EntrepriseRepository $entrepriseRep): Response
    {
        return $this->render('nhema/caisse/transfert_fond/show.html.twig', [
            'transfert_fond' => $transfertFond,
            'entreprise' => $entrepriseRep->find(1),
            'lieu_vente' => $lieu_vente,
        ]);
    ...
    #[Route('/edit/{id}/{lieu_vente}', name: 'app_nhema_caisse_transfert_fond_edit', methods: ['GET', 'POST'])]
    public function edit(Request $request, TransfertFond $transfertFond, EntityManagerInterface $entityManager, LieuxVentes $lieu_vente, EntrepriseRepository $entrepriseRep, TransfertFondRepository $transfertRep, MouvementCaisseRepository $mouvementRep): Response
    {
        $form = $this->createForm(TransfertFondType::class, $transfertFond, ['lieu_vente' => $lieu_vente]);
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (empty($caisse_depart) and empty($caisse_recep)) {
                ...
                $mouvement_caisse = $mouvementRep->findOneBy(['transfertFond' => $transfertFond]); 
                ...
                if ($solde_caisse >= $montant) {
                    ...
                    if (!empty($caisse_depart)) {
                        $mouvement_caisse_depart = $mouvement_caisse;
                        $mouvement_caisse_depart->setMontant(-$montant)
                                ->setSaisiePar($this->getUser())
                                ->setDevise($form->getViewData()->getDevise())
                                ->setCaisse($form->getViewData()->getCaisseDepart())
                                ->setLieuVente($lieu_vente)
                    ...
                    if (!empty($caisse_recep)) {
                        ...
                        if ($caisse_recep->gettype() != 'banque' ) {
                            if ($caisse_recep->getPointDeVente()->getLieuVente() == $lieu_vente) {
                                ...
                                $mouvement_caisse_recep = $mouvement_caisse;
                                $mouvement_caisse_recep->setMontant($montant)
                                        ->setDevise($form->getViewData()->getDevise())
                                        ->setCaisse($form->getViewData()
                                        ->setSaisiePar($this->getUser())->getCaisseReception())
                                        ->setLieuVente($lieu_vente)
                                        ->setDateOperation($form->getViewData()->getDateOperation())
                                        ->setDateSaisie(new \DateTime("now"));
                                $transfertFond->addMouvementCaiss($mouvement_caisse_recep);
        ...
        return $this->render('nhema/caisse/transfert_fond/edit.html.twig', [
            'transfert_fond' => $transfertFond,
            'form' => $form,
            'entreprise' => $entrepriseRep->find(1),
            'lieu_vente' => $lieu_vente,
    ...
    #[Route('/delete/{id}/{lieu_vente}', name: 'app_nhema_caisse_transfert_fond_delete', methods: ['POST'])]
    public function delete(Request $request, TransfertFond $transfertFond, EntityManagerInterface $entityManager, LieuxVentes $lieu_vente, EntrepriseRepository $entrepriseRep): Response
    {
    ...
    #[Route('/delete/reception/{id}/{lieu_vente}', name: 'app_nhema_caisse_transfert_fond_delete_reception', methods: ['POST'])]
    ...
    public function receptionTransfert(TransfertFondRepository $transfertFondRepository, Request $request, CaisseRepository $caisseRep, LieuxVentes $lieu_vente, EntrepriseRepository $entrepriseRep, CompteOperationRepository $compteOpRep, MouvementCaisseRepository $mouvementRep, CategorieOperationRepository $catetgorieOpRep, EntityManagerInterface $em): Response
    {
        if ($request->get('caisse_recep')) {
            ...
            return new RedirectResponse($this->generateUrl('app_nhema_caisse_transfert_fond_reception', ['lieu_vente' => $lieu_vente->getId()]));
        }
        if ($request->get("search")){
            $search = $caisseRep->find($request->get("search"));
        }else{
        ...
        return $this->render('nhema/caisse/transfert_fond/reception.html.twig', [
            'transfert_fonds' => $transferts,
            'entreprise' => $entrepriseRep->find(1),
            'lieu_vente' => $lieu_vente,
            'search' => $search,
