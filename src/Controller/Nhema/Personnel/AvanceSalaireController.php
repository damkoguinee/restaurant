...
#[Route('/nhema/personnel/avance/salaire')]
class AvanceSalaireController extends AbstractController
{
    #[Route('/{lieu_vente}', name: 'app_nhema_personnel_avance_salaire_index', methods: ['GET'])]
    ...
    {
        return $this->render('nhema/personnel/avance_salaire/index.html.twig', [
    ...
    #[Route('/new/{lieu_vente}', name: 'app_nhema_personnel_avance_salaire_new', methods: ['GET', 'POST'])]
    public function new(Request $request, LieuxVentes $lieu_vente, UserRepository $userRep, DeviseRepository $deviseRep, CategorieOperationRepository $catOpRep, CompteOperationRepository $compteOpRep, CaisseRepository $caisseRep, EntityManagerInterface $entityManager, EntrepriseRepository $entrepriseRep, MouvementCaisseRepository $mouvementRep): Response
    {
        ...
        if (isset($request->request->all()['avance_salaire']['periode'])) {
            ...
            if (empty($periode) or empty($montant) or empty($caisse)) {
                return $this->redirectToRoute('app_nhema_personnel_avance_salaire_new', ['lieu_vente' => $lieu_vente->getId()], Response::HTTP_SEE_OTHER);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($solde_caisse >= $montant) {
                ...
                return $this->redirectToRoute('app_nhema_personnel_avance_salaire_index', ['lieu_vente' => $lieu_vente->getId()], Response::HTTP_SEE_OTHER);
            }else{
                ...
                if ($referer) {
                    ...
                    return $this->render('nhema/personnel/avance_salaire/new.html.twig', [
                        'entreprise' => $entrepriseRep->find(1),
                        'lieu_vente' => $lieu_vente,
    ...
    #[Route('/show/{id}/{lieu_vente}', name: 'app_nhema_personnel_avance_salaire_show', methods: ['GET'])]
    public function show(AvanceSalaire $avanceSalaire, LieuxVentes $lieu_vente, EntrepriseRepository $entrepriseRep): Response
    {
        return $this->render('nhema/personnel/avance_salaire/show.html.twig', [
            'avance_salaire' => $avanceSalaire,
            'entreprise' => $entrepriseRep->find(1),
            'lieu_vente' => $lieu_vente,
        ]);
    ...
    #[Route('/edit/{id}/{lieu_vente}', name: 'app_nhema_personnel_avance_salaire_edit', methods: ['GET', 'POST'])]
    public function edit(Request $request, AvanceSalaire $avanceSalaire, EntityManagerInterface $entityManager, LieuxVentes $lieu_vente, EntrepriseRepository $entrepriseRep, MouvementCaisseRepository $mouvementRep,DeviseRepository $deviseRep, CaisseRepository $caisseRep,): Response
    {
        $form = $this->createForm(AvanceSalaireType::class, $avanceSalaire, ['lieu_vente' => $lieu_vente]);
        $form->handleRequest($request);
        ...
        return $this->render('nhema/personnel/avance_salaire/edit.html.twig', [
            'avance_salaire' => $avanceSalaire,
            'form' => $form,
            'entreprise' => $entrepriseRep->find(1),
            'lieu_vente' => $lieu_vente,
    ...
    #[Route('/{id}/{lieu_vente}', name: 'app_nhema_personnel_avance_salaire_delete', methods: ['POST'])]
